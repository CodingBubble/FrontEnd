...
class _StateTransaction_Create extends State<Transaction_Create> {
  ...
  void initState() {
    ...
    reload_user_options().then((value) => 
      setState(() {
        ...
        children: <Widget>[
          ...
          Padding(
              ...
              child: TextButton(
                ...
                child: Text('Transaktion erstellen', style: new TextStyle(color: variationColor)),
              )),
        ],
      )
    );
  ...
  {
    if (current_transaction_group==null) { return; }
    ...
    if (selected_user1==null){
      for (User u1 in users_in_group) {
        imply_transaction(title_controller.text, u1, balance/users_in_group.length);
      }
    }
    else {
  ...
  Future imply_transaction(String title, User u1, double balance) async {
    if (selected_user2==null){
      for (User u2 in users_in_group) {
        ...
        do_transaction(title_controller.text, u1, u2, balance/users_in_group.length);
      }
    }
    else {
